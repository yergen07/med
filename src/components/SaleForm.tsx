...
export const SaleForm: React.FC = () => {
  const { user } = useAuth();
  const { getManagerStock, createSale } = useInventory();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      createSale(
        ...
        formData.productId,
        quantity,
        0, // amount set to 0 since we removed it
        '', // customerName empty
        '', // customerPhone empty
        '', // customerCity empty
        formData.date,
      ...
      setMessage({ type: 'success', text: 'Продажа успешно оформлена' });
      setFormData({
        productId: '',
        quantity: '',
        date: new Date().toISOString().split('T')[0],
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-gray-900">Оформление продажи</h2>
        <p className="text-gray-600 mt-1">Внесение информации о продаже товара</p>
      </div>
      ...
      <div className="bg-green-50 border border-green-200 rounded-lg p-4">
        <div className="flex items-start">
          <ShoppingCart className="h-5 w-5 text-green-600 mt-0.5 mr-3" />
          <div>
            <h3 className="text-sm font-medium text-green-800">Информация о продаже</h3>
            <div className="mt-2 text-sm text-green-700">
              <ul className="list-disc list-inside space-y-1">
                <li>Товар будет списан с вашего склада</li>
                <li>Продажа будет зафиксирована в системе</li>
